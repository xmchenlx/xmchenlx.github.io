...
在2021年的10月1日前，可以通过[bishe.chenlx.top](http://bishe.chenlx.top)进行项目的预览。在这个时间过后，这个域名将会因为过期而撤销。
...
main文件是项目的配置文件，主要是对插件的引用和一些参数的配置。
...
这个控件的显示效果比上一款树形表格的显示效果好一些，但是不支持拖拽。
...
在频繁迭代的Axure图里，总会出现非常相似的页面布局。所以这部分相似的页面布局，为了不重复写这些布局代码，就用JSON配置的方式，让页面在渲染时，通过v-for的方法生成页面的每一个控件。一般来说，用JSON生成页面的组件，开头都会有一段注释来进行说明。
...
在开发环境，输入`npm run dev`进行启动。设定的访问地址为`http://localhost:8080`。根据实际端口占用的情况可能会变更。
